...
if (!alive _unit) then {
		...
		"Select Loadout",
		{
			...
			switch _mode do {
				...
				case "select": {
					...
					if (_selected == missionnamespace getvariable ["BIS_fnc_respawnMenuInventory_selected",""] && _selected != "") exitwith {};
				...
				case "update": {
					...
					if (count _respawnInventories > 0) then {
						...
						_cursel = 0;
						{
							...
							if !(isclass _cfg) then {_cfg = configfile >> "cfgvehicles" >> _x;};
							if !(isclass _cfg) then {
								...
								if (count _xSplit == 2) then {
									...
									if (count _cfg == 0) then {_valid = false;};
								} else {
									_valid = false;
								};
							};
							if (_valid) then {
								...
								if (isnil {_var}) then {
									...
									_displayName = "";
									_icon = "";
									if (typename _cfg == typename configfile) then {
										...
										_displayName = _cfg select 1;
										_icon = gettext (configfile >> "RscDisplayArsenal" >> "icon");
									};
									...
									_magazines = magazines _previewObject;//getarray (_cfg >> "magazines");
									_magazineTypes = [];
									_magazineCounts = [];
	...
	if !(isclass _cfg) then {
		...
		_vehicle = gettext (_cfg >> "vehicle");
		[_unit,_cfg] call bis_fnc_loadInventory;
		if (_vehicle != "") then {[_unit,_vehicle,getarray (_cfg >> "vehicleIgnore")] call bis_fnc_loadInventory;};
	};
