...
#define TASK_ID				"BIS_infantry_challenge_8"
#define REQUIRED_HANDGUN 		"Pistol_Base_F"
#define REQUIRED_RIFLE	 		"Rifle_Base_F"
#define MINIMAL_RIFLE			"arifle_Mk20_ACO_pointer_F"
#define MINIMAL_HANDGUN			"hgun_ACPC2_F"
#define REQUIRED_SMOKE 			"SmokeShell"
#define REQUIRED_GRENADE	 	"HandGrenade"
#define REQUIRED_AMOUNT_SMOKES		1
#define REQUIRED_AMOUNT_GRENADES	1
#define CHALLENGE_TIMEOUT		120
...
switch (toLower _action) do {
	...
	case (toLower "conditionSucceed") : {
		{ currentWeapon _x != "" } count (["allRecruits"] call BIS_fnc_flow) == count (["allRecruits"] call BIS_fnc_flow);
	};
	...
	case (toLower "onFail") : {
		...
		// Add the stuff to recruits manually
		{
			// Throwables
			_x addMagazineGlobal REQUIRED_SMOKE;
			_x addMagazineGlobal REQUIRED_GRENADE;
			
			// Magazines
			for "_i" from 0 to 1 do {
				_x addMagazineGlobal ((getArray (configFile >> "CfgWeapons" >> MINIMAL_HANDGUN >> "Magazines")) select 0);
				_x addMagazineGlobal ((getArray (configFile >> "CfgWeapons" >> MINIMAL_RIFLE >> "Magazines")) select 0);
			};
			
			// Weapons
			_x addWeaponGlobal MINIMAL_HANDGUN;
			_x addWeaponGlobal MINIMAL_RIFLE;
