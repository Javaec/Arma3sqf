/*
	Training: Throwing Grenades
*/

// Defines
#define STAGE_ID		"infantry"
#define TASK_ID 		"BIS_infantry_challenge_4"
#define RANGE_MARKER		"BIS_firingRange_grenade"
#define RANGE_RADIUS		50
#define SHOTS_MIN_COUNT		3
...
switch (toLower _action) do {
	...
	case (toLower "update") : {
		...
		if (isNil { BIS_instructorRecruitStatus } || { count BIS_instructorRecruitStatus < 1 } || { time >= (BIS_instructorRecruitStatus select 3) + _publicDelay }) then {
			BIS_instructorRecruitStatus = [
				...
				{ count (["getRecruitBestCourse", [BIS_grenadesFiringRange_board, getPlayerUid _this, SHOTS_MIN_COUNT]] call BIS_fnc_firingRange) <= 0 },
				time
			];
			publicVariable "BIS_instructorRecruitStatus";
		};
	...
	case (toLower "conditionSucceed") : {
		{ count (["getRecruitBestCourse", [BIS_grenadesFiringRange_board, getPlayerUid _x, SHOTS_MIN_COUNT]] call BIS_fnc_firingRange) > 0 } count (["allRecruits"] call BIS_fnc_flow) == count (["allRecruits"] call BIS_fnc_flow);
	};
