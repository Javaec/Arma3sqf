...
#define TASK_ID 		"BIS_motorized_challenge_2"
#define VEHICLE			"Quadbike_01_base_F"
...
switch (toLower _action) do {
	...
	case (toLower "update") : {
		...
		if (isNil { BIS_instructorRecruitStatus } || { count BIS_instructorRecruitStatus < 1 } || { time >= (BIS_instructorRecruitStatus select 3) + _publicDelay }) then {
			BIS_instructorRecruitStatus = [
				...
				{ !(vehicle _this isKindOf VEHICLE) || { _this != driver vehicle _this } },
				time
			];
			publicVariable "BIS_instructorRecruitStatus";
		};
	...
	case (toLower "conditionSucceed") : {
		{ vehicle _x isKindOf VEHICLE && { _x == driver vehicle _x } } count (["allRecruits"] call BIS_fnc_flow) >= count (["allRecruits"] call BIS_fnc_flow);
	};
