/*
	Training: Firing from a vehicle
*/

// Defines
#define STAGE_ID			"motorized"
#define TASK_ID	 			"BIS_motorized_challenge_6"
#define SHOTS_MIN_COUNT			5
#define REQUIRED_WEAPON			"HMG_127"
...
switch (toLower _action) do {
	...
	case (toLower "update") : {
		...
		if (isNil { BIS_instructorRecruitStatus } || { count BIS_instructorRecruitStatus < 1 } || { time >= (BIS_instructorRecruitStatus select 3) + _publicDelay }) then {
			BIS_instructorRecruitStatus = [
				...
				{ count (["getRecruitBestCourse", [BIS_vehiclesFiringRange_board, getPlayerUid _this, SHOTS_MIN_COUNT, REQUIRED_WEAPON]] call BIS_fnc_firingRange) <= 0 },
				time
			];
			publicVariable "BIS_instructorRecruitStatus";
		};
	...
	case (toLower "conditionSucceed") : {
		{ count (["getRecruitBestCourse", [BIS_vehiclesFiringRange_board, getPlayerUid _x, SHOTS_MIN_COUNT, REQUIRED_WEAPON]] call BIS_fnc_firingRange) > 0 } count (["allRecruits"] call BIS_fnc_flow) == count (["allRecruits"] call BIS_fnc_flow);
	};
