...
switch _mode do
{
	...
	case "onLoad":
	{
		...
		//Mark unclickable until it has appID
		//_control = _display displayctrl IDC_RSCDISPLAYDLCCONTENTBROWSER_MARKSMENOVERLAY;//IDC_RSCDISPLAYDLCCONTENTBROWSER_MARKSMENLOGO;
		//_control ctrladdeventhandler ["buttonclick","with uinamespace do {['dlcButtonClicked',_this,''] call RscDisplayDLCContentBrowser_script};"];
		...
		//Marksmen button - blue, unclickable, Coming Soon
		_control = _display displayctrl IDC_RSCDISPLAYDLCCONTENTBROWSER_MARKSMENBACKGROUND;
		_control ctrlSetBackgroundColor ([(configfile >> "CfgMods" >> "Marksman"), "dlcColor", [0.35, 0.35, 0.1, 1]] call bis_fnc_returnConfigEntry);
	...
	case "checkDLCOwnership":
	{
		...
		{ //for each class from cfgMods
			...
			switch(_appId) do
			{
				...
				//Mark appID is unknown
				// case xxxxxx:
				// {
					//Mark
					// IDC_RSCDISPLAYDLCCONTENTBROWSER_MARKSMENBACKGROUND
				// };
			};
