...

{
	...
	_x allowFleeing 0;
	_x setUnitPos "UP";
	_x addMagazines ["30Rnd_65x39_caseless_mag_Tracer", 5];
	_x addWeapon "arifle_MX_Black_F";
	_x spawn {
		while {TRUE} do {
			waitUntil {count magazines _this < 3};
			_this addMagazines ["30Rnd_65x39_caseless_mag", 2];
		};
	};
...

{
	_x spawn {
		...
		while {TRUE} do {
			...
			_unit = "O_Soldier_VR_F" createUnit [markerPos _this, _grp, "if (dayTime < 1) then {this addWeapon 'NVGoggles'}; this addVest 'V_TacVest_blk'; this addHeadgear 'H_HelmetB_black'; this addMagazines ['30Rnd_556x45_Stanag_Tracer_Green', 5]; this addWeapon 'arifle_SDAR_F';", 0.2];
			_unit = leader _grp;
